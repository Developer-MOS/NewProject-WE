...
export function EmailProvider({ children }: { children: React.ReactNode }) {
  ...
  // Instead of handling token, redirect to backend OAuth route
  const handleLogin = () => {
  ...
  const fetchEmails = async () => {
    ...
    try {
      ...
      console.error("Error fetching emails:", error);
      setAuthError("Failed to fetch emails.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const toggleStar = async (emailId: string) => {
    try {
      await axios.post(`http://localhost:3000/gmail/messages/${emailId}/star`);
      setEmails((prev) =>
  ...
  const markAsRead = async (emailId: string) => {
    try {
      await axios.post(`http://localhost:3000/gmail/messages/${emailId}/read`);
      setEmails((prev) =>
  ...
  // console.log(emails,'This is the emails list . . . . ');
